...
# === Stub ops pre-load: provide missing C++ symbols for librtp_compute_ops.so ===
_stub_so = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "..", "librtp_compute_ops_stub.so")
if not os.path.exists(_stub_so):
    _stub_so = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "libs", "librtp_compute_ops_stub.so")
if os.path.exists(_stub_so):
    try:
        from ctypes import CDLL
        import os as _os
        CDLL(_stub_so, mode=_os.RTLD_GLOBAL | _os.RTLD_NOW | _os.RTLD_NODELETE)
        _logging.info(f"pre-loaded stub {_stub_so} with RTLD_GLOBAL")
    except BaseException as e:
        _logging.warning(f"Failed to pre-load stub so: {e}")
# === End stub ops pre-load ===
