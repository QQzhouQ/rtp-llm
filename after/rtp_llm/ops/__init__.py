...
_stub_so = os.path.join(libs_path, "librtp_compute_ops_stub.so")
if os.path.exists(_stub_so):
    try:
        from ctypes import CDLL
        CDLL(_stub_so, mode=os.RTLD_GLOBAL | os.RTLD_NOW | os.RTLD_NODELETE)
        logging.info(f"pre-loaded stub {_stub_so} with RTLD_GLOBAL")
    except BaseException as e:
        logging.warning(f"Failed to pre-load stub so: {e}")
